...
abstract: "Este guia rápido foi criado como parte do Projeto Meros do Brasil para direcionar a calibração do nosso sistema Stereo-BRUV. Portanto, cada BRUV representa um set up de duas câmeras, uma esquerda e outra direita, de mesmo modelo e configuração. Para uso consulte os termos."
...
5.  É para esse momento que serve o bater palmas ou os dedos (movimentos de *clap*). Servindo de claquete para marcar o início da tomada, para gerarmos oo sincronismo audiovisual entre imagem e som.
    ...
    Trata-se de um esquecimento comum durante as operações de campo, pois a dinâmica da atividade embarcada associada a um conjunto de passos que precisam ser seguidos antes do lançamento, nos leva a esquecer dos procedimentos mais simples.

    A realização dessa operação é fundamental para a sincronização posterior das câmeras dentro do programa de fotogrametria, seja para tomada de medida ou na etapa de calibração.

    A não realização ou esquecimento dessa etapa pode inviabilizar toda a amostragem do dia, desperdiçando assim nosso esforço de campo, financeiro e físico.

    Ainda que possível realizar a sincronização na ausência deste movimento, a operação será muito mais trabalhosa, despedindo muito mais capricho e tempo para a realização de uma simple etapa.

    Atente-se que por se tratar de um sistema de **stereo**-BRUV, com **câmeras em diferentes ângulos**, você pode acabar obstruindo a visualização de uma delas a depender da sua posição em relação a elas.

    **Se faz necessário que ambas as câmeras registre o movimento "clap".**
    :::

    ::: {.callout-tip appearance="simple"}
    **A dica é utilizar movimentos rápidos e precisos.**

    **Sugiro o movimento de pinçar os dedos, como um caranguejo, batendo de forma rápida o polegar contra o indicador várias vezes seguidas de modo a aumentarmos a chance de captura limpa.**
    :::
...
2.  Se você receber uma mensagem de aviso sobre medições de distância excluídas ou baixa precisão relativa, ignore-a por enquanto.
    ...
    ![](images/clipboard-2851610986.png)
...
3.  Se o ajuste falhar, consulte os motivos pelos quais um ajuste de feixe pode falhar. Se ainda estiver com dificuldades para obter resultados que não convergem, tente uma solução apenas com a face frontal, descrita em Ajustes com falha em um cubo de calibração SeaGIS.
    ...
    ![](images/clipboard-2208317373.png)

4.  Clique em *Acepted results*.
